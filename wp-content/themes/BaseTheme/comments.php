...
if (!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME'])) /*1*/
		die (__('Please do not load this page directly. Thanks!','basetheme21_td'));
if ( post_password_required() ) { /*2*/ ?>
	<?php _e('This post is password protected. Enter the password to view comments.','basetheme21_td'); ?>
<?php
	return;
}
		...
		if ( have_comments() ) : ?>
			...
			<?php
				...
				if ( '1' === $theme_comment_count ) {
					printf(
						/* translators: 1: title. */
						__( 'One Responce', 'basetheme21_td' ),
						'<span>' . get_the_title() . '</span>'
					);
				} else {
					printf( // WPCS: XSS OK.
						/* translators: 1: comment count number, 2: title. */
						esc_html( _nx( '%1$s Responce', '%1$s Responces', $theme_comment_count, 'comments title', 'basetheme21_td' ) ),
			...
			if ( ! comments_open() ) :
				?>
				<p class="no-comments"><?php _e( 'Comments are closed.', 'basetheme21_td' ); ?></p>
				<?php
			endif;
